...
if [ ! -f /var/www/html/installer ]; then
    ...
    jinja2 /var/www/html/.xe_install_config.j2 \
     ...
     -D database_charset=$XE_INSTALL_DATABASE_CHARSET \
     -D composer_home_directory=$XE_COMPOSER_HOME_DIRECTORY \
     > /var/www/html/.xe_install_config.yaml

    php -r "copy('http://start.xpressengine.io/download/installer', 'installer');" && php installer install --config=/var/www/html/.xe_install_config.yaml --no-interact
fi
